...
class DaemonManager {
  DaemonManager({String? Function()? homeDirProvider})
      : _homeDirProvider = homeDirProvider;

  Process? _daemonProcess;
  bool _isRunning = false;
  bool _isRetrying = false;
  DateTime? _lastStartAttempt;
  final String? Function()? _homeDirProvider;
  ...
  String get daemonPath {
    ...
    if (Platform.isMacOS) {
      ...
      bundledPath =
          path.join(contentsDir, 'Resources', 'bin', 'transmission-daemon');
  ...
  String getDownloadDir(String? customDir) {
    ...
    if (Platform.isWindows) {
      // Windows uses %USERPROFILE%\Music by default
      musicDir = path.join(homeDir, 'Music');
    } else if (Platform.isMacOS || Platform.isLinux) {
      musicDir = path.join(homeDir, 'Music');
    } else {
      // Fallback: place alongside home dir
      musicDir = path.join(homeDir, 'Music');
    }
  ...
  String? _resolveHomeDir() {
    final provided = _homeDirProvider?.call();
    ...
    try {
      if (Platform.isWindows) {
        ...
        final result = await Process.run(
            'tasklist', ['/FI', 'IMAGENAME eq transmission-daemon.exe', '/NH']);
        return result.exitCode == 0 &&
            result.stdout.toString().contains('transmission-daemon.exe');
      } else {
        // Unix/Linux/macOS: use pgrep
        final result =
            await Process.run('pgrep', ['-x', 'transmission-daemon']);
        return result.exitCode == 0;
      }
    } catch (e) {
      return false;
  ...
  Future<bool> startDaemon({String? customDownloadDir}) async {
    ...
    try {
      ...
      try {
        ...
        throw Exception('Cannot access config directory:\n$config\n\n'
      ...
      try {
        ...
        print(
            '[Daemon] ✓ Download directory accessible and writable: $download');
      } on FileSystemException catch (e) {
        ...
        if (Platform.isMacOS) {
          protectedFolderTip =
              'Tip: macOS may require granting TrustTune access to the Music folder (System Settings → Privacy & Security → Files and Folders).';
        } else if (Platform.isWindows) {
          protectedFolderTip =
              'Tip: Avoid system folders like Program Files, Windows, or protected locations.';
        } else {
          protectedFolderTip =
              'Tip: Avoid system folders like /usr, /bin, or root-owned directories.';
        }
      ...
      for (int attempt = 1; attempt <= 3; attempt++) {
        try {
          final socket = await Socket.connect('127.0.0.1', 9091,
            ...
            if (Platform.isWindows) {
              ...
              if (!_isRetrying && _lastStartAttempt != null) {
                ...
                if (timeSinceStart.inSeconds < 10) {
                  print(
                      '[Daemon] 🔄 Auto-retrying startup (UAC prompt may have interrupted)...');
                  _isRetrying = true;
                  await Future.delayed(const Duration(seconds: 2));
                  await startDaemon();
                  _isRetrying = false;
  ...
  Future<void> stopDaemon() async {
    ...
    try {
      if (Platform.isWindows) {
        ...
        print(
            '[Daemon] Killed all transmission-daemon.exe processes (Windows)');
      } else {
      ...
      print(
          '[Daemon] Note: Error killing system-wide processes (may not exist): $e');
    }
